...
`check.mjs` (runner) + `rules.json` manifest + `rules/` (71 rules, v2.2.0).
...
- `check.mjs` + `rules.json` + `rules/` + `src/` — the runner, the manifest, and the 71 rules (co-located; keep them together).
