...
> **⚠ AMENDED (M3 — sliced, 2026-07-10):** M3 (issue #20) is ~3–4× a normal module, so it ships in slices. **M3a (shipped): `baseline orient`** — the unified `baseline.mjs` CLI (`orient` new; `check` delegates to the intact check.mjs), the capability probe, and the derived-state survey generalizing the ADR-0009 `orient.mjs` (capability header · divergence-first · live lanes · backlog · this-lane `next:`), plus the Claude Code SessionStart hook + SKILL first-act directive. `gh`-based, degrades gracefully (FS9), dogfooded on baseline-skill's own forge. **M3b (shipped):** the typed facts layer + `.baseline/cache` (advisory-only) + `join` over declared keys (`schema/keys.md`) + `derive/status` + forge record/replay fixtures, with `orient` refactored onto `facts→join→derive`. **M3c (shipped):** the per-rule `sources/on_unreachable/contexts/certainty` backfill + the self-check structural laws (blocker⇒deterministic, sign-off⇒judgment) + CTX-12 (stored-status tripwire, warn). **M3d (shipped, conformance-level):** the `baseline-orient` Hermes plugin (`integrations/hermes/`) — an `on_session_start` hook + `/orient` command over the official NousResearch hermes-agent `register(ctx)` API (the plan's `prefetch`/`system_prompt_block` names were memory-provider-specific; the real hook is `on_session_start`). Authored to spec; a live-Hermes runtime demo of the session-start injection is the one open item. **M3 (issue #20) is now shipped end-to-end** across four slices.

> **⚠ AMENDED (M4 — sliced + panel ruling, 2026-07-11):** M4 (issue #21) ships in three slices after an adversarial panel pass (scope-cutter / friction skeptic / dependency auditor — all AMENDS; ruling record: the #21 amendment comment). Corrections to the module's own text: the "break-glass JDG via its own prior PR" line is the **consumer-runtime FS5 discipline enforced at M6 admit** — M4 ships the break-glass-*expressible* judgment schema (kind + `gate`) and CONTRACT.md documents the discipline, no intra-M4 PR ceremony; the "REC-02 blocker" wording resolves to **rule lands warn** (with deterministic-pattern metadata so M7's promotion is a pure severity flip) while blocking comes from **tooling gates** — `baseline log`'s write-time refusal and the pre-push hook — honoring blocker⇒deterministic; REC rules land in **`rules/rec.json`** (per §7), not `rules/flow.json`. **M4a (shipped):** the per-category `rules/` split behind a manifest loader (corpus-neutral; #22's premise), the four record schemas + templates + the shared subset validator (`src/validate.mjs`), `src/scrub.mjs` (one scan API: SEC-01-parity deterministic tier blocks, assignment/entropy heuristics warn, dated allowlist judgments keyed by content-derived finding id), and **`baseline log`** with the pinned UX — one command, lane/agent/timestamp derived (lane = branch incl. unborn, the M5 seam), stdin accepted, never $EDITOR; CF1 collision-free paths via O_EXCL; scrub blocks non-lossy (draft under `.baseline/cache/` + exact rerun; inline `--allow <id> --allow-reason`). **M4b (shipped):** the unified JDG ledger — `baseline jdg new|check` over a pure evaluator (worst-wins lattice TRIPPED>EXPIRED>UNRESOLVABLE>DRIFTED>OK; ops eq/ne/gt/lt/exists/absent; unresolvable fact = surfaced finding, never a guess), facts namespace `descriptor.*`/`planes.*`/`git.*`/`today` + `--facts` overlay (M6's sweep reuses it); the DESC-03-shape fixture validates + fires on posture weakening (schema-validation only — rule enforcement stays M6); the signoff→JDG bridge (unexpired `kind: sign-off` by subject satisfies the manual rule, a lapsed one is honestly NOT signed, legacy signoff.json dual-reads until M7) pinned by the new `jdg-repo` golden fixture; CONTRACT.md ships (the C28 plain-git twin incl. the FS5 break-glass prior-PR discipline); plus the deferred M4a consolidations (one argv parser, one frontmatter boundary — fixing doc-freshness's CRLF blindness). **M4c (shipped):** REC-01/02/04/05 (rules/rec.json; REC-05 = the push-gate delegation) + FLOW-02/06 record-coupled advisory (SKIP-gated on single-lane posture/default branch — no wallpaper warns), pre-push hook scaffold + push-protection/gitleaks delegation, claims explosion + `gen migrate-claims` (dual-read; legacy-read retirement filed onto #24's delete list), `status_file: false` honored with a descriptor present, one corpus re-pin. Dogfood hardened: the baseline-demo catch-up (descriptor + orient adoption, overdue since 2026-07-05) starts alongside M4a; issue-level acceptance is **≥3 organic sessions, ≥1 organic JDG, zero unclearable warns on baseline-demo**.

> **⚠ AMENDED (M5 — sliced + panel ruling, 2026-07-13):** M5 (issue #22) ships in **three slices** after the same adversarial panel as M4 (scope-cutter / friction skeptic / dependency auditor — **all three AMEND**; this block is the ruling record; the #22 mirror comment posts on Adar's approval). Load-bearing amendments: **the ref IS the claim and carries no slug** — `lanes.namespace` with the issue number substituted, exactly (`lane/<N>`), because two slug spellings would both push-succeed and mint two lanes for one issue (checkbox 1's "exactly one wins" is a property of the refname); **claim is checkout-free** — `fetch` → `commit-tree` (empty commit, `Baseline-Issue`/`Baseline-Agent` trailers validated against descriptor `join_keys`, its real consumer) → `push <sha>:refs/heads/<ref>`, so the loser exits 3 having never touched HEAD/worktree/branches — no-residue is structural, and on push rejection the CLI re-asks origin to distinguish lost-race (exit 3) from transport failure (exit 2, git's reason, never a fake race); descriptor field flips are **earned per slice** (M2's honest-slice law): `lanes` + `join_keys` flip at M5a, `owner` has **no M5 consumer** (lane identity is the trailer) and moves to #24's consume-or-drop list; **FS10 is amended on verified fact** — GitHub's GraphQL schema no longer carries `Commit.pushedDate`, so lease freshness = max(tip committedDate, PR updatedAt), provenance-labeled, erring toward LIVE (safe against premature reclaim), fetched as ONE GraphQL `refs()` query; **LIVE|STALE|ABANDONED stays three-state in the view** (C31) with STALE = ttl/2 as a named provisional constant (no descriptor knob; M7 revisits on dogfood data) while the **liveness rule warns only at ABANDONED**; `lane reclaim` refuses unless derived ABANDONED, takes over via trailer commit + **machine-written takeover record through the existing `baseline log` writer** (the issue's "dated takeover record" with no forced human ceremony) + best-effort issue comment, and **`--jdg <id>`** honors an unexpired deviation naming the lane (the live-takeover escape hatch, so nobody routes around the tool); FLOW rules land warn + promotion metadata with **no overlap and no wallpaper** — FLOW-01 anchoring checks existence + resolution only (open-ness is DIV's alone), FLOW-03 fires only on a present record with empty `next:` (absence stays FLOW-02's), FLOW-04 (a scope addition, capped, first-to-drop) checks namespace placement against `lanes.namespace` + new additive `lanes.families` (the demo's real branch inventory — adopt/*, release/* — must not be wallpaper), FLOW-05 push discipline is the **arbitrated threshold-free predicate** "newest session record exists locally but is absent at origin" (session-boundary-aligned, zero invented tuning surface), FLOW-07 lease liveness; DIV-01..03 come from extracted `derive/divergence` and **DIVERGED lands as a real engine tag** (CATS+div, report TAG, summary.diverged; certainty deterministic, severity warn, exit unchanged until M7) because checkbox 4's "on fixtures" cannot be pinned orient-only; the golden harness grows `_fixture.json` `forge_replay`/`bare_origin` so lane/DIV verdicts are pinnable; `check` gains the **capability-probe + forge-facts plumbing it entirely lacks today** (the panel's blocker: no FLOW/DIV rule can evaluate in check without it), exit-stable offline; multi-lane-local SKIPs say **"forge not consulted (multi-lane-local posture)"** — the posture is named, never faked as unreachability. **M5a (shipped in its PR): claim** — `workflow` enum + `multi-lane-local` (CF5) with **`makeForge` owning the posture closure** (a posture-closed forge stays closed even under replay — one home for the label, M5b/M5c inherit), the claim primitive as above with posture-gated issue verification (positively-closed refuses as divergence-at-birth naming the reopen; unverifiable or stateless proceeds labeled; multi-lane-local never consults), **race settled honestly in both directions** (an existing ref under this agent's own trailer = idempotent win; a push whose report is lost after origin applied the ref = win by tip==sha, never a fake loss), the claim base fetched into a private ref (FETCH_HEAD never read — concurrent-fetch-proof), absent `join_keys` refusing like an incomplete one (C38), origin-HEAD fallback when `default_branch` is undeclared (asked, not guessed), single-branch clones opting the lane into the fetch refspec (detected by the set-upstream refusal), the two-clone concurrent race test on a local bare origin (checkbox 1, structural) plus idempotency/rejecting-origin/no-origin/no-descriptor coverage, and an 8-angle pre-merge review with all confirmed findings fixed in-branch; corpus untouched (no rule changes — FLOW-02/06's family-array conversion waits for M5c's one re-pin). **M5b (shipped in its PR): leases + reclaim + orient** — forge lane-refs query (record/replay-additive), git-plane fallback labeled low-confidence, pure `derive/lanes` (+ anchor resolution, the M6 `inputs_digest` seam), skew clamps to age 0 labeled, fresh-claim-derives-LIVE pinned, reclaim as above, orient lane lines gain state/age/agent with labels riding the line and ABANDONED/STALE sorted first (claimed-but-PR-less lanes appear — C31). **M5c (shipped in its PR): rules + verdicts + ONE re-pin** — check's facts plumbing, engine `workflow` string-or-array + selfcheck enum lockstep, FLOW-01/03/04/05/07 + DIV-01..03 as above, contexts `["check","admit"]`, `lanes-repo` golden fixture, docs (REFERENCE/CHANGELOG/SKILL/CONTRACT). After release: demo re-pin declaring `lanes.families` + the live two-agent claim race on a demo issue — checkbox 1's real-repo proof, organic sessions for #21. GraphQL batching stays deferred (one refs query per run; F2's fleet-scale trigger hasn't fired).
