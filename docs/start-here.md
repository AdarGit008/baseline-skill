...
last-verified: 46b5748 2026-07-13
...
`check.mjs` (runner) + `rules.json` manifest + `rules/` (78 rules, v2.2.0 line).
...
- `check.mjs` + `rules.json` + `rules/` + `src/` — the runner, the manifest, and the 78 rules (co-located; keep them together).
