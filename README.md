...
against **92 rules** across build, tests, security & [supply-chain](GLOSSARY.md#supply-chain), reproducibility,
...
<picture>
  ...
  <img alt="How /baseline decides — the evaluate stack. Five layers: the CLI (check.mjs) loads 92 rules as pure data; the judge (engine.mjs) gates and tags; the lab (evaluators.mjs) runs 41 check kinds; the senses (repo.mjs) read files and git; the world is fs + git itself. Verdicts PASS/FAIL/WARN/DIVERGED/SKIP/SIGN-OFF roll up into one exit code that gates CI." src="docs/assets/evaluate-stack-light.svg" width="100%">
</picture>
...
| `rules.json` | the rule-set manifest (version, profiles, module list) — the 92 rules live in `rules/` |
