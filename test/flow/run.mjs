...
// The ambient env must not steer the tool under test (the admit harness's lesson): a CI
// pull_request job exports GITHUB_HEAD_REF/REF_NAME/WORKSPACE for every step, and the #55
// cases below assert precisely what check does with those. Leaked, they would both mask
// failures and invent them. Strip; every case re-injects what it means to test.
...
const git = (cwd, ...a) => execFileSync('git', ['-C', cwd, ...a], { encoding: 'utf8', env: { ...CLEAN_ENV, ...GITENV } }).trim()
...
function world(name, { desc = {}, replay = null } = {}) {
  ...
  const bare = path.join(dir, 'origin.git'); execFileSync('git', ['init', '-q', '--bare', '-b', 'main', bare], { env: { ...CLEAN_ENV, ...GITENV } })
...
function checkJson(w, { replayDir = null, env = {} } = {}) {
  const r = spawnSync(process.execPath, [BASELINE, 'check', '--repo', w, '--no-exec', '--json'],
    { encoding: 'utf8', env: { ...CLEAN_ENV, ...GITENV, ...(replayDir ? { BASELINE_FORGE_REPLAY: replayDir } : {}), ...env } })
...
const LANE_FAMILY = ['FLOW-02', 'FLOW-03', 'FLOW-04', 'DIV-02']
{
  ...
  const pr = checkJson(w, { env: { GITHUB_WORKSPACE: w, GITHUB_HEAD_REF: 'lane/1', GITHUB_EVENT_NAME: 'pull_request' } })
  ...
  const merge = checkJson(w, { env: { GITHUB_WORKSPACE: w, GITHUB_HEAD_REF: 'lane/1', GITHUB_REF_TYPE: 'branch', GITHUB_REF_NAME: '7/merge', GITHUB_EVENT_NAME: 'pull_request' } })
  ...
  const tagpush = checkJson(w, { env: { GITHUB_WORKSPACE: w, GITHUB_REF_TYPE: 'tag', GITHUB_REF_NAME: 'v1.2.3', GITHUB_EVENT_NAME: 'push' } })
  for (const id of LANE_FAMILY) ok(tag(tagpush, id).tag === 'SKIP', `${id}: a tag push is not a lane (REF_TYPE=tag is refused)`)
  const detpush = checkJson(w, { env: { GITHUB_WORKSPACE: w, GITHUB_REF_TYPE: 'branch', GITHUB_REF_NAME: 'lane/1', GITHUB_EVENT_NAME: 'push' } })
  ...
  const junk = checkJson(w, { env: { GITHUB_WORKSPACE: w, GITHUB_HEAD_REF: '   ', GITHUB_EVENT_NAME: 'pull_request' } })
  ok(junk.lane?.name == null && tag(junk, 'FLOW-02').tag === 'SKIP', 'an environment naming nothing resolves to null, never to a lane called ""')

  // the event speaks for the workspace the JOB checked out, not for every repo the job
  // scores. CI caught this on #55's own PR: a suite scoring a temp fixture inherited the
  // job's GITHUB_HEAD_REF and labeled an unrelated repo with the PR's branch.
  const foreign = checkJson(w, { env: { GITHUB_WORKSPACE: path.join(w, '..', 'somewhere-else'), GITHUB_HEAD_REF: 'lane/1', GITHUB_EVENT_NAME: 'pull_request' } })
  ok(foreign.lane?.name == null && tag(foreign, 'FLOW-02').tag === 'SKIP',
    `a CI event whose GITHUB_WORKSPACE is a DIFFERENT repo names no lane here (got ${JSON.stringify(foreign.lane)})`)

  // the checkout always wins: a stale exported HEAD_REF cannot redirect a real branch
  git(w, 'checkout', '-q', 'lane/1')
  const stale = checkJson(w, { env: { GITHUB_WORKSPACE: w, GITHUB_HEAD_REF: 'lane/999', GITHUB_EVENT_NAME: 'pull_request' } })
